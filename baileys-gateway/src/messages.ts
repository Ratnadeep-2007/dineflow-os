...
export async function sendDineInMenu(
  ...
  const baseUrl = process.env.MENU_DASHBOARD_URL ?? 'https://dineflow-kiosk.onrender.com/menu';
...
export async function sendOrderConfirmed(sock: WASocket, jid: string, orderRef: string): Promise<void> {
  await sendText(
    ...
    `_Type */bot/HI* or *Dinein* anytime to start a new order._`,
  );
}
...
export async function sendInvalidInput(sock: WASocket, jid: string): Promise<void> {
  await sendText(
    ...
    `Reply with *1* for Dine In, *2* for Takeaway, *3* for Delivery, or type */bot/HI* to start over.`,
  );
}
